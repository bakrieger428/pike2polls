import React, { HTMLAttributes, forwardRef } from 'react';

/**
 * Alert Component
 *
 * Accessible alert/notification component.
 * Meets WCAG 2.1 AA standards including:
 * - Color contrast minimum 4.5:1
 * - Proper ARIA live regions for announcements
 * - Icon indicators with aria-hidden
 * - Dismissible with keyboard
 */

export type AlertVariant = 'info' | 'success' | 'warning' | 'error';

export interface AlertProps extends HTMLAttributes<HTMLDivElement> {
  /**
   * Alert variant/style
   * @default 'info'
   */
  variant?: AlertVariant;

  /**
   * Alert title
   */
  title?: string;

  /**
   * Allow dismissal
   * @default false
   */
  dismissible?: boolean;

  /**
   * Callback when dismissed
   */
  onDismiss?: () => void;

  /**
   * Use ARIA live region for announcements
   * @default true
   */
  live?: boolean;

  /**
   * Politeness setting for live region
   * @default 'polite'
   */
  liveLevel?: 'polite' | 'assertive';
}

const variantConfig: Record<AlertVariant, { icon: string; label: string }> = {
  info: {
    icon: `<svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
      <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
    </svg>`,
    label: 'Information',
  },
  success: {
    icon: `<svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
      <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
    </svg>`,
    label: 'Success',
  },
  warning: {
    icon: `<svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
      <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
    </svg>`,
    label: 'Warning',
  },
  error: {
    icon: `<svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
      <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" />
    </svg>`,
    label: 'Error',
  },
};

/**
 * Alert Component
 *
 * @example
 * ```tsx
 * <Alert variant="info">
 *   This is an informational message.
 * </Alert>
 *
 * <Alert variant="success" title="Success!">
 *   Your ride has been scheduled.
 * </Alert>
 *
 * <Alert variant="error" title="Error" dismissible onDismiss={handleDismiss}>
 *   Something went wrong. Please try again.
 * </Alert>
 *
 * <Alert variant="warning" live liveLevel="assertive">
 *   Important announcement that interrupts users.
 * </Alert>
 * ```
 */
export const Alert = forwardRef<HTMLDivElement, AlertProps>(
  (
    {
      variant = 'info',
      title,
      dismissible = false,
      onDismiss,
      live = true,
      liveLevel = 'polite',
      className = '',
      children,
      role,
      ...props
    },
    ref
  ) => {
    const config = variantConfig[variant];
    const alertClasses = `alert alert-${variant} ${className}`.trim();

    // Set role - use 'alert' for assertive, 'status' for polite (default)
    const alertRole = role || (liveLevel === 'assertive' ? 'alert' : 'status');

    return (
      <div
        ref={ref}
        className={alertClasses}
        role={alertRole}
        aria-live={live ? liveLevel : undefined}
        aria-atomic={live ? 'true' : undefined}
        {...props}
      >
        {/* Icon */}
        <span className="flex-shrink-0" aria-hidden="true">
          <span dangerouslySetInnerHTML={{ __html: config.icon }} />
        </span>

        {/* Content */}
        <div className="flex-1 min-w-0">
          {title && (
            <p className="font-semibold text-body-md mb-1">
              {title}
            </p>
          )}
          <div className="text-body-md">
            {children}
          </div>
        </div>

        {/* Dismiss button */}
        {dismissible && (
          <button
            type="button"
            onClick={onDismiss}
            className="flex-shrink-0 p-1 rounded hover:bg-black/10 focus-visible:outline-focus-ring"
            aria-label="Dismiss notification"
          >
            <svg
              className="w-5 h-5"
              fill="currentColor"
              viewBox="0 0 20 20"
              aria-hidden="true"
            >
              <path
                fillRule="evenodd"
                d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
                clipRule="evenodd"
              />
            </svg>
          </button>
        )}
      </div>
    );
  }
);

Alert.displayName = 'Alert';
