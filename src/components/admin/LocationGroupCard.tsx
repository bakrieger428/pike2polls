/**
 * LocationGroupCard Component
 *
 * Displays a single location group with:
 * - Group address and rider count
 * - List of riders at this address
 * - Driver assignment status
 * - Assign driver button
 * - View route manifest button
 *
 * This is adapted from RiderGroupCard but for the new hierarchical
 * organization where groups are by exact address.
 *
    ...
    <Card className="p-6 mb-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
        <div className="flex-grow min-w-0">
          <h3 className="text-heading-md font-semibold text-text-primary mb-1 truncate">
            {group.streetAddress || group.formattedAddress}
          </h3>
          <p className="text-body-sm text-text-secondary">
            {formatVotingDate(group.votingDate)} at {group.preferredTime}
          </p>
        </div>
      ...
      {driverAssignment && assignedDriver && (
        <div className="mb-4 p-3 bg-success-50 border border-success-200 rounded-md">
          <div className="text-body-sm text-text-secondary">
            ...
            <button
              type="button"
              onClick={() => onViewDriverManifest(driverAssignment.volunteer_id, group.votingDate)}
        ...
        {group.riders.map((rider, index) => (
          ...
          >
            ...
            <div className="flex-grow min-w-0">
              ...
              {rider.formattedAddress && rider.formattedAddress !== group.streetAddress && (
                <div className="text-body-sm text-text-tertiary mt-1">
                  {rider.formattedAddress}
                </div>
              )}
